import { a8 as __exportAll, g as getRequestContext, ae as isI18nEnabled, af as getFallbackChain, k as requestCached, aa as encodeCursor, a9 as isMissingTableError, ag as getI18nConfig } from './adapt-sandbox-entry_H5gl0boC.mjs';
import { C as CURSOR_RAW_VALUES } from './loader-ou_PXAjg_XDqWmGlU.mjs';
...
async function getEmDashCollectionUncached(type, filter) {
	const { getLiveCollection } = await import('./_astro_content_Du9deACq.mjs');
...
async function getEmDashEntry(type, id, options) {
	const { getLiveEntry } = await import('./_astro_content_Du9deACq.mjs');
	const ctx = getRequestContext();
...
async function hydrateEntryBylines(type, entries) {
	...
	try {
		const { getBylinesForEntries } = await import('./bylines-DTFI8nDM_WDUzvaAX.mjs').then((n) => n.t);
...
async function hydrateEntryTerms(type, entries) {
	if (entries.length === 0) return;
	try {
		const { getAllTermsForEntries } = await import('./taxonomies-JmQQZiG1_DH1Y73oi.mjs').then((n) => n.u);
...
async function getTranslations(type, id) {
	try {
		const db = (await import('./loader-ou_PXAjg_XDqWmGlU.mjs').then(n => n.l).then((n) => n.i)).getDb;
		const dbInstance = await db();
		const { ContentRepository } = await import('./adapt-sandbox-entry_H5gl0boC.mjs').then(n => n.aq).then((n) => n.n);
...
async function resolveEmDashPath(path) {
	if (!cachedUrlPatterns) {
		const { getDb } = await import('./loader-ou_PXAjg_XDqWmGlU.mjs').then(n => n.l).then((n) => n.i);
		const { SchemaRegistry } = await import('./adapt-sandbox-entry_H5gl0boC.mjs').then(n => n.ar).then((n) => n.r);
