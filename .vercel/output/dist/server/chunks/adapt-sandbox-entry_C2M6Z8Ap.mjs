...
async function applySeed(db, seed, options = {}) {
	...
	const { invalidateBylineCache } = await import('./bylines-DTFI8nDM_C4wjDES8.mjs').then((n) => n.t);
	const { invalidateRedirectCache } = await Promise.resolve().then(() => cacheBAJbeoZ8).then((n) => n.t);
	const { invalidateUrlPatternCache } = await import('./query-yA3-rFji_DDWmMCXT.mjs').then((n) => n.o);
...
async function applyContentTaxonomies(db, collectionSlug, contentId, entry, isUpdate) {
	...
	if (!entry.taxonomies) {
		if (isUpdate) {
			const { invalidateTermCache } = await import('./taxonomies-JmQQZiG1_BgxQ8EyO.mjs').then((n) => n.u);
			invalidateTermCache();
		}
		return;
	}
